...
from utils.model_utils import equivariant_layer, invariant_layer, sample_dosages, sample_Z




def sample_X(X, ps, size):
    ps = np.array(ps) / sum(ps)
    ids = np.linspace(0,X.shape[0]-1,X.shape[0],dtype='int')
    start_idx = np.random.choice(ids, size, p=ps)
    return start_idx

def ealry_stop(current, best_it, tolerance):
    if (current - best_it) > tolerance:
        return True
    else:
        return False
...
class SCIGAN_Model:
    def __init__(self, params, dataset_name='reg'):
        if 'seda' in dataset_name:
    ...
    def train(self, Train_X, Train_T, Train_D, Train_Y, Train_ps, verbose=False,\
              tolerance=1000):
        ...
        for it in tqdm(range(self.epochs)):
            for kk in range(2):
                idx_mb = sample_X(Train_X, Train_ps, self.batch_size)
                X_mb = Train_X[idx_mb, :]
            ...
            if best_loss > MSE:
                best_loss = MSE
                best_it = it
                #saver.save(self.sess, 'generator')
##                tf.saved_model.simple_save(self.sess, export_dir=self.export_dir,
...
##                                             outputs={'inference_outcome': I_logits})
            if ealry_stop(it, best_it, tolerance):
                break
            # %% Debugging
...
##                print()
        ...
        #saver = tf.train.import_meta_graph('generator.meta')
        ...
        for it in tqdm(range(self.epochs)):
            ...
            current_I = float(I_loss_curr)
            

            if best_loss > current_I:
                best_loss = current_I
                best_it = it
                #saver.save(self.sess, 'my_test_model',global_step=1000)
