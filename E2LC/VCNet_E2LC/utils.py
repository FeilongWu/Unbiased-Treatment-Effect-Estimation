...
def get_opt_samples(data_tr, density_model, parameters, size, std_w=3):
    ...
    inv_density = {}
    n = len(parameters)
    cdf = torch.linspace(0,1,size).tolist()
    density_model.eval()
    train_size = 0
    for i in range(n):
        a,b = parameters[i]
        ...
        samples[i].insert(0, 0.0)
        samples[i].append(1.0)
        inv_density[i].insert(0, 1.0)
        inv_density[i].append(1.0)
        errors[i] = 0
    ...
    for idx, batch in enumerate(data_tr):
        ...
        for j in range(size):
            propensity = density_model.log_prob(torch.tensor([samples[n][j]]*bs).float()\
                                                    .unsqueeze(-1)-0.5,x)
            errors[n] += 1/size * np.sum(1 / np.exp(propensity.detach().numpy())) * inv_density[n][j]
