'''
discretized regimes for response curve using top 1% similar patients.
add penalty function to control the 
'''
import numpy as np
import csv
...
def fun(x,a,b,c,d):
    return sigmoid(a * x + b * x**2 + c * x**0.5 - d)

def penalty_fun(x,a,b,c,d, constrain=[0,0,0,0]):
    inputs = [x,a,b,c,d]
    output = fun(*inputs)
    penalty = 0
...
def fit_curve(data, t, density_model,upper=10,lower=0,\
    ...
    if plot_single:
        ...
        top_k_t = np.append(top_k_t, 0)
        top_k_y = np.append(top_k_y, 0)

        initial_parameters = np.array([0.1, 0.1, 0.1, 0])
        ###
        if use_IPW:
            ### top_k_y reweight
            ...
            top_k_y = np.clip(np.array(top_k_y) / temp, 0, 1) # use clip for sedatives
        ...
        if use_IPW:
            save_pth = 'mimiciv_seda_response_curve_calibrate_IPW.pickle'
            ...
            save_pth = 'mimiciv_seda_response_curve_calibrate.pickle'
            if os.path.isfile(save_pth):
                with open(save_pth, 'rb') as file:
                    result = pickle.load(file)
                    start = len(result) + 1
...
if __name__ == '__main__':
    path_lable = './mimiciv_seda_label.csv'
    t_min, t_max_min = 0.04545454, 0.9791356094
    y_mu = 0.0
    y_std = 1.0

    path = './mimiciv_seda.csv'
    ...
    plot_single = True
    patient_idx = 100 # this is used only when plot_single = True
                    # otherwise, plot average all
    use_IPW = False
    
    regimes = [(0.0,0.17),(0.17,0.35),(0.35,0.7),(0.7,1.0)]
